...
class trafficlight:
    def __init__(self,
        ...
        self.init_phase()
        self.ob_space = spaces.Box(low=np.zeros(self.num_green_phases+1+1+2*len(self.lanes), dtype=np.float32), high=np.ones(self.num_green_phases+1+1+2*len(self.lanes), dtype=np.float32))
    ...
    def get_state(self):
        ...
        max_green = [0 if self.time_since_last_change > self.max_green + self.yellow_time else 1]
        density = self.get_lane_density()
        queue = self.get_lane_queue()
        state = np.array(phase + min_green + max_green + density + queue, dtype=np.float32)
        return state
    ...
    def set_next_phase(self, newphase):
        if (self.time_since_last_change < self.max_green + self.yellow_time) and (self.green_phase == newphase or self.time_since_last_change < self.min_green + self.yellow_time):
            self.max_green = 0
            self.sumo.trafficlight.setRedYellowGreenState(self.id, self.all_phases[self.green_phase].state)
            self.next_action_time = self.env.sim_step + self.delta_time
        else:
            while self.green_phase == newphase:
                pdb.set_trace()
                newphase = random.choice(self.green_phases)
